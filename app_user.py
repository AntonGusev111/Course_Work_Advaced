...
class User():
    ...
    def attribute_check(self) -> list:  # дописать чекатрибут
        check_list = []
        if self.attribute_list['sex'] == '':
            check_list.append('пол')
        if self.attribute_list['city'] == '':
            check_list.append('город')
        if self.attribute_list['bdate'] == '':
            check_list.append('дата рождения')
        if not check_list:
